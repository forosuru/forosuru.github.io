...
def write_stats_file():
    ...
    for k in users:
        ...
        user_img = '<img src="%s" style="margin-right: 5px; width: 20px; height: 20px">' % (v['avatar_url'], )
        ...
        user_link += ' href="{{ site.this_site }}/user/%s.html">%s%s</a></div>' % (v['login'], user_img, v['login'])
        ec = v['event_counts']
        row = '<tr> <td>%s</td> <td>%d</td> <td>%d</td> <td>%d</td> <td>%d</td> <td>%d</td> </tr>' % (user_link, ec['repo'], ec['follow'], ec['fork'], ec['public'], ec['watch_started'])
        data.append(row)
